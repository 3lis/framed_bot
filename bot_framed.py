...
#####################################################################################################################
    ...
    Alice Apr 2022   ~   Bot Telegram for Framed Leaderboard
...
# unicodes used in the message generated by framed
camera              = "\U0001F3A5"
block_green         = "\U0001F7E9"
block_red           = "\U0001F7E5"
block_black         = "\U00002B1B"

# structure of the head of the message generated by framed
pattern             = f"^(Framed #[0-9]+\n{camera}"
pattern            += f"[{block_green}{block_red}{block_black}' ']{{12}})"
...
def start_conv( update, context ):
    ...
    txt     = "Play [Framed](https://framed.wtf/) and share your score! "
...
def save_score( update, context ):
    """ -------------------------------------------------------------------------------------------------------------
    Save the score of the day. User must send a message generated by the Framed website
    ------------------------------------------------------------------------------------------------------------- """
    user        = update.effective_user.username
    if not check_user( user, update ):
        return
    ...
    if r is None:
        txt     = "ERROR: the bot only accepts messages generated by Framed"
        msg( update, txt )
        return
